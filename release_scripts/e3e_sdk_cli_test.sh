...
# one workspace.
# - If you want to test interactive login, do not set leopard_RELEASE_CREDENTIALS,
...
echo "Logging in..."
if [ -n "${leopard_RELEASE_CREDENTIALS}" ]; then
    lep login -c "$leopard_RELEASE_CREDENTIALS"
else
    lep login
fi
...
leopard_WS_TOKEN=$(lep workspace token)
leopard_WS_URL=$(lep workspace url)
echo "Done"
echo

echo "Obtaining leopard sdk test folder..."
SCRIPT_PATH=$(realpath "$0")
SCRIPT_FOLDER=$(dirname "$SCRIPT_PATH")
leopard_TEST_FOLDER=$SCRIPT_FOLDER/../leopardai/tests
echo "Path for leopard test folder: $leopard_TEST_FOLDER"
echo "Done"
echo
...
command="lep photon create -n ${COMMON_NAME} -m $leopard_TEST_FOLDER/shell/shell.py"
...
command="${CURL} -f -s -X 'POST' ${leopard_WS_URL}/run \
              -H 'Content-Type: application/json' \
              -H 'accept: application/json' \
              -H 'X-leopard-Deployment: ${COMMON_NAME}' \
              -H 'Authorization: Bearer ${leopard_WS_TOKEN}' \
              -d '{\"query\": \"echo yes\"}'"
...
command="${CURL} -f -s -X 'POST' ${leopard_WS_URL}/run \
              -H 'Content-Type: application/json' \
              -H 'accept: application/json' \
              -H 'X-leopard-Deployment: ${COMMON_NAME}-with-env' \
              -H 'Authorization: Bearer ${leopard_WS_TOKEN}' \
...
command="lep photon run -n ${COMMON_NAME} -dn ${COMMON_NAME}-with-storage --mount /:/mnt/leopardstore"
if eval "$command" > /dev/null; then
    echo "Done"
else
    echo "lep photon run failed. This should not happen. Reproduce with: $command"
...
command="${CURL} -f -s -X 'POST' ${leopard_WS_URL}/run \
              -H 'Content-Type: application/json' \
              -H 'accept: application/json' \
              -H 'X-leopard-Deployment: ${COMMON_NAME}-with-storage' \
              -H 'Authorization: Bearer ${leopard_WS_TOKEN}' \
              -d '{\"query\": \"touch /mnt/leopardstore/${COMMON_NAME}.txt\"}'"
...
command="${CURL} -f -s -X 'POST' ${leopard_WS_URL}/run \
              ...
              -H 'X-leopard-Deployment: ${COMMON_NAME}-with-storage' \
              -H 'Authorization: Bearer ${leopard_WS_TOKEN}' \
              -d '{\"query\": \"ls /mnt/leopardstore/${COMMON_NAME}.txt\"}'"
...
command="${CURL} -f -s -X 'POST' ${leopard_WS_URL}/run \
              ...
              -H 'X-leopard-Deployment: ${COMMON_NAME}-with-storage' \
              -H 'Authorization: Bearer ${leopard_WS_TOKEN}' \
              -d '{\"query\": \"rm /mnt/leopardstore/${COMMON_NAME}.txt\"}'"
if eval "$command" > /dev/null; then
