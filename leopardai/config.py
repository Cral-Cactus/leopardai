"""
Overall configurations and constants for the leopard AI python library.
"""
...
# Cache directory for leopard's local configurations.
...
# `leopard_CACHE_DIR`, BEFORE IMPORTING leopardAI.
#
...
CACHE_DIR = Path(os.environ.get("leopard_CACHE_DIR", Path.home() / ".cache" / "leopard"))

################################################################################
# Configurations you can change to customize leopard's behavior.
################################################################################
...
# and run such tokenizers. Set the environment variable `leopard_TRUST_REMOTE_CODE`
# to `false` to disable this behavior.
TRUST_REMOTE_CODE = os.environ.get("leopard_TRUST_REMOTE_CODE", "true").lower() in (
    "true",
    "1",
    "t",
    "on",
...
# preferred in a development environment. Set the environment variable `leopard_DEFAULT_TIMEOUT`
# to `false` to disable this behavior.
_DEFAULT_TIMEOUT_IF_NOT_SPECIFIED = 3600
if os.environ.get(
    "leopard_DEFAULT_TIMEOUT", str(_DEFAULT_TIMEOUT_IF_NOT_SPECIFIED)
    ...
    try:
        ...
        print(
            f"You have set an invalid value for leopard_DEFAULT_TIMEOUT {timeout_str}."
...
# leopard_DEFAULT_TIMEOUT to false, we still want to set the default timeout inside
# cloudrun, because it was expected to be in-process.
try:
    CLOUDRUN_DEFAULT_TIMEOUT = int(
        os.environ.get("leopard_CLOUDRUN_DEFAULT_TIMEOUT", "3600")
    )
except ValueError:
    CLOUDRUN_DEFAULT_TIMEOUT = 3600  # 1 hour
    print(
        "You have set an invalid value for leopard_CLOUDRUN_DEFAULT_TIMEOUT"
        f" {os.environ.get('leopard_CLOUDRUN_DEFAULT_TIMEOUT')}. Using default value"
...
if pydantic.version.VERSION < "2.0.0":
    ...
    warnings.warn(
        "You are using pydantic 1.x, which is not fully supported by leopard AI. We"
...
################################################################################
# leopard internals. Do not change these as they will change the behavior of the
# library and APIs.
################################################################################

# Cache directory for leopard's local storage: database, logs, etc.
# To change the cache directory, set the environment variable leopard_CACHE_DIR before importing leopardai.
...
#
# If you are in need of accessing the cache directory, use the functions `create_cached_dir_if_needed()` in `leopardai.utils`.
CACHE_DIR = Path(os.environ.get("leopard_CACHE_DIR", Path.home() / ".cache" / "leopard"))
DB_PATH = CACHE_DIR / "leopard.db"
...
def _is_rocm() -> bool:
    ...
    If the user specified leopard_BASE_IMAGE_FORCE_ROCM to any true values, then
    ...
    user code.
    """
    return _to_bool(os.environ.get("leopard_BASE_IMAGE_FORCE_ROCM", "false"))


# leopard's base image and image repository location.
BASE_IMAGE_VERSION = "0.21.1"
BASE_IMAGE_REGISTRY = "default"
BASE_IMAGE_REPO = f"{BASE_IMAGE_REGISTRY}/leopard"
BASE_IMAGE = (
...
# Default port used by the leopard deployments.
DEFAULT_PORT = 8080

try:
    DEFAULT_TIMEOUT_KEEP_ALIVE = int(
        os.environ.get("leopard_TIMEOUT_KEEP_ALIVE", "1800")
    )
except ValueError:
    DEFAULT_TIMEOUT_KEEP_ALIVE = 1800  # 30 minutes
    print(
        "You have set an invalid value for leopard_TIMEOUT_KEEP_ALIVE."
...
DEFAULT_TIMEOUT_GRACEFUL_SHUTDOWN = (
    int(os.environ["leopard_TIMEOUT_GRACEFUL_SHUTDOWN"])
    if os.environ.get("leopard_TIMEOUT_GRACEFUL_SHUTDOWN")
    else None
)
...
FORCE_PIP_INSTALL_PYDANTIC_AND_CLOUDPICKLE = _to_bool(
    os.environ.get("leopard_FORCE_PIP_INSTALL_PYDANTIC_AND_CLOUDPICKLE", "false")
)
...
if os.environ.get("leopard_INCOMING_TRAFFIC_GRACE_PERIOD"):
    # If the user has explicitly set the grace period, we will use the user's setting.
    DEFAULT_INCOMING_TRAFFIC_GRACE_PERIOD = int(
        os.environ["leopard_INCOMING_TRAFFIC_GRACE_PERIOD"]
    )
elif "leopard_WORKSPACE_ID" in os.environ and "leopard_DEPLOYMENT_NAME" in os.environ:
    # Else, if we are running on the leopard platform, we will use the default grace period
...
def set_local_deployment_token(token: str):
    """
    Sets the local token used by leopard deployments. If the token is empty, we will not
    ...
    Note that on the leopard platform, the token is managed by the platform, so we
    ...
    if "leopard_WORKSPACE_ID" in os.environ and "leopard_DEPLOYMENT_NAME" in os.environ:
        warnings.warn(
            "We are running on the leopard platform, so we will ignore the local"
            " deployment token."
        )
    else:
...
def get_local_deployment_token() -> str:
    ...
    return the token. Otherwise, return the token from the environment variable leopard_LOCAL_DEPLOYMENT_TOKEN.
    If the environment variable is not set, return an empty string.
    """
    if _LOCAL_DEPLOYMENT_TOKEN is None:
        return os.environ.get("leopard_LOCAL_DEPLOYMENT_TOKEN", "")
    else:
        return _LOCAL_DEPLOYMENT_TOKEN


# In the photon's deployment template, this means you will need to specify env variables.
ENV_VAR_REQUIRED = "PLEASE_ENTER_YOUR_ENV_VARIABLE_HERE_(leopard_ENV_VAR_REQUIRED)"
...
WORKSPACE_URL_RESOLVER_API = "https://portal.leopard.ai/api/workspace"

# Current workspace api path
WORKSPACE_API_PATH = "/api/v1"

# leopard reserved secret and env prefix. One is not supposed to use this in `--env` or `--secret` flags.
leopard_RESERVED_ENV_NAMES = {
    "leopard_WORKSPACE_ID",
    "leopard_DEPLOYMENT_NAME",
    "leopard_PHOTON_NAME",
    "leopard_PHOTON_ID",
    "leopard_RESOURCE_ACCELERATOR_TYPE",
}

if "leopard_ALLOW_ORIGINS" in os.environ:
    ALLOW_ORIGINS = os.environ["leopard_ALLOW_ORIGINS"].split(",")
else:
    ALLOW_ORIGINS = ["*"]

leopard_DASHBOARD_URL = "https://dashboard.leopard.ai"
# leopard_WORKSPACE_URL is used to get the web url for the workspace. Append "/dashboard" for the workspace dashboard.
leopard_WORKSPACE_URL = leopard_DASHBOARD_URL + "/workspace/{workspace_id}"
# leopard_DEPLOYMENT_URL is used to get the web url for the deployment.
# Append "/demo", "/api", "/metrics", "/events", "/replicas/list" for the deployment dashboard functions.
leopard_DEPLOYMENT_URL = leopard_WORKSPACE_URL + "/deployments/detail/{deployment_name}"