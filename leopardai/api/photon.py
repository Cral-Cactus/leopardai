...
from leopardai.config import CACHE_DIR, ENV_VAR_REQUIRED, leopard_RESERVED_ENV_NAMES

# import leopardai.photon to register the schemas and types
import leopardai.photon  # noqa: F401
from leopardai.photon.base import (
    add_photon,
    remove_local_photon,
    find_all_local_photons,
)
from leopardai.photon.util import load
...
def make_env_vars_from_strings(
    ...
    for s in env if env else []:
        ...
        if k in leopard_RESERVED_ENV_NAMES:
            raise ValueError(
                "You have used a reserved environment variable name that is "
                "used by leopard internally: {k}. Please use a different name. "
                "Here is a list of all reserved environment variable names:\n"
                f"{leopard_RESERVED_ENV_NAMES}"
            )
