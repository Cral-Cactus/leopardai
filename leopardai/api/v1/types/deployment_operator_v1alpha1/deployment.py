...
from leopardai.config import compatible_field_validator, v2only_field_validator
...
class ContainerPort(BaseModel):
    """
    The port spec of a leopard Job.
    """
...
class leopardContainer(BaseModel):
    """
    The container spec of a leopard Job.
    """
...
class leopardMetrics(BaseModel):
    """
    The metrics spec of a leopard Job.
    """
...
class leopardDeploymentUserSpec(BaseModel):
    ...
    log: Optional[leopardLog] = None
    metrics: Optional[leopardMetrics] = None
    routing_policy: Optional[leopardRoutingPolicy] = None


class leopardDeploymentState(str, Enum):
    Ready = "Ready"
    NotReady = "Not Ready"
    Starting = "Starting"
    Updating = "Updating"
...
class leopardDeploymentStatus(BaseModel):
    state: leopardDeploymentState
