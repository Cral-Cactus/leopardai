...
class WorkspaceRecord(object):
    ...
    def login_with_env(cls):
        ...
        """
        workspace_id = os.environ.get("leopard_WORKSPACE_ID")
        auth_token = os.environ.get("leopard_WORKSPACE_TOKEN")
        if workspace_id:
            cls.set(workspace_id, auth_token)
        else:
            raise RuntimeError(
                "leopard_WORKSPACE_ID environment variable is not set. Please set it to"
