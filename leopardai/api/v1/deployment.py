...
class DeploymentAPI(APIResourse):
    def _to_name(self, name_or_deployment: Union[str, leopardDeployment]) -> str:
        return (  # type: ignore
            name_or_deployment
    ...
    def list_all(self):
        ...
        return self.ensure_list(response, leopardDeployment)

    def create(self, spec: leopardDeployment):
        """
    ...
    def create_pod(self, spec: leopardDeployment):
        """
        Creates a pod with the given deployment spec. This is equivalent to creating a deployment
        with is_pod=True.
        """
        warnings.warn(
            "create_pod is deprecated. Use the api under leopardai.api.v1.pod instead,"
            " which is more explicit and gives more strict param checking.",
            DeprecationWarning,
        )
        if spec.spec is None:
            raise ValueError("leopardDeploymentUserSpec must not be None.")
        spec.spec.is_pod = True
        # todo: pod-specific fields check if needed.
        return self.create(spec)

    def get(self, name_or_deployment: Union[str, leopardDeployment]) -> leopardDeployment:
        response = self._get(f"/deployments/{self._to_name(name_or_deployment)}")
        return self.ensure_type(response, leopardDeployment)

    def update(
        self, name_or_deployment: Union[str, leopardDeployment], spec: leopardDeployment
    ) -> leopardDeployment:
    ...
    def delete(self, name_or_deployment: Union[str, leopardDeployment]) -> bool:
        response = self._delete(f"/deployments/{self._to_name(name_or_deployment)}")
        return self.ensure_ok(response)

    def restart(
        self, name_or_deployment: Union[str, leopardDeployment]
    ) -> leopardDeployment:
        response = self._put(
    ...
    def get_log(
        self,
        name_or_deployment: Union[str, leopardDeployment],
