...
from leopardai.api.util import _get_full_workspace_api_url
...
class APIClient(object):
    """
    A leopard API client that is associated with a workspace. This class holds all
    ...
    def __init__(
        ...
        # - user specified one
        # - environment variable leopard_WORKSPACE_TOKEN
        # - auth token of the workspace record
        auth_token = (
            auth_token
            or os.environ.get("leopard_WORKSPACE_TOKEN")
            or (
        ...
        url = (
            url
            or os.environ.get("leopard_WORKSPACE_URL")
            or (
        ...
        if os.environ.get("leopard_DEBUG_HEADERS"):
            # leopard_DEBUG_HEADERS should be in the format of comma separated
            # header_key=header_value pairs.
            try:
                header_pairs = os.environ["leopard_DEBUG_HEADERS"].split(",")
                ...
                raise RuntimeError(
                    ...
                    f" {os.environ['leopard_DEBUG_HEADERS']}"
                )
