...
from .types.deployment import leopardDeployment, leopardDeploymentUserSpec
from .types.readiness import ReadinessIssue
from .types.termination import DeploymentTerminations


class PodAPI(APIResourse):
    def _to_name(self, name_or_pod: Union[str, leopardDeployment]) -> str:
        return (  # type: ignore
            name_or_pod if isinstance(name_or_pod, str) else name_or_pod.metadata.id_
        )

    def _sanity_check_pod_spec(self, spec: Optional[leopardDeploymentUserSpec]):
        """
    ...
    def list_all(self) -> List[leopardDeployment]:
        response = self._get("/deployments")
        deployments = self.ensure_list(response, leopardDeployment)
    ...
    def get(self, name_or_pod: Union[str, leopardDeployment]) -> leopardDeployment:
