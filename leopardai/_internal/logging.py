"""
Internal logging module for leopard. logs are written to
"logs/internal.log" inside the leopard directory. Once enabled, logs
triggered by
  - regular `logger.{debug,info,warning,error}` call will be written
  to both the log file and printed to the console;
  - `leopardai._internal.logging.log` call will only be written to the
...
def enable():
    ...
    leopardai.config.LOGS_DIR. enable() and disable() can be called multiple times to
    temporarily turn on and off internal logging.

    Note: internal logging requires writing to filesystems, and as a result, we require
    the user to explicitly set an env variable "leopard_ENABLE_INTERNAL_LOG" to 1 or true.
    Otherwise, it will not be enabled even when enable() is called.
    """
    global _enabled
    global _HANDLER_ID

    if os.environ.get("leopard_ENABLE_INTERNAL_LOG", "0").lower() not in ("1", "true"):
        return
