...
from leopardai.api.photon import make_mounts_from_strings, make_env_vars_from_strings
from leopardai.config import BASE_IMAGE

from leopardai.api.v1.types.common import Metadata
from leopardai.api.v1.types.job import (
    leopardJob,
    leopardJobUserSpec,
    leopardResourceAffinity,
)
from leopardai.api.v1.types.deployment import ContainerPort
...
def job():
    """
    Manages leopard Jobs.

    leopard Jobs are for one-time and one-off tasks that run on one or more machines.
...
@click.option(
    ...
    type=str,
    help=(
        "Container image for the job. If not set, default to leopardai.config.BASE_IMAGE"
    ),
    default=None,
)
@click.option(
...
def create(
    ...
    if file:
        try:
            with open(file, "r") as f:
                content = f.read()
                job_spec = leopardJobUserSpec.parse_raw(content)
        except Exception as e:
            console.print(f"Cannot load job spec from file [red]{file}[/]: {e}")
            return
    else:
        job_spec = leopardJobUserSpec()
    ...
    if node_groups:
        ...
        job_spec.affinity = job_spec.affinity or leopardResourceAffinity()
        job_spec.affinity = leopardResourceAffinity(
    ...
    job = leopardJob(spec=job_spec, metadata=Metadata(id=name))
