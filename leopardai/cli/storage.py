...
def storage():
    """
    Manage File storage on the leopard AI cloud.

    leopard AI provides a file storage service that allows you to store files and
...
def upload(local_path, remote_path, rsync, recursive, progress):
    ...
    if rsync:
        ...
        name = "storage-rsync-by-leopard"

        leopard_deployment = client.deployment.get(name)
        port = leopard_deployment.spec.container.ports[0].host_port
