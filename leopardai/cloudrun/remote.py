...
from leopardai.photon.util import create as create_photon
from leopardai import api, config
from leopardai.api import APIError
from leopardai.client import Client, current
...
class Remote(object):
    ...
    def __init__(self, photon: Union[Type[Photon], str, Photon], **kwargs):
        ...
        Args:
            ...
            **kwargs: Keyword arguments to create the deployment. Currently, they are:
            ...
            See leopardai.api.deployment.run_remote for details.
        """
    ...
    def _run_photon(self, resume: bool = False):
        ...
        logger.debug(
            "Go to https://dashboard.leopard.ai/ to check detailed progress of the"
        ...
        while not is_deployment_ready and time.time() - start < self._MAX_WAIT_TIME:
            ...
            if isinstance(ret, APIError):
                ...
                # will use "Ready". ref: https://github.com/leopardai/leopard/pull/3152
