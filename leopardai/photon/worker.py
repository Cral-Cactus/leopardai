...
from leopardai.util import asyncfy_with_semaphore
...
class Worker(Photon):
    ...
    # deployment, make sure to set the leopard_WORKSPACE_TOKEN environment variable as the
    ...
    leopard_TASK_ID_TAG = "_leopard_task_id"
    ...
    def init(self):
        ...
        from leopardai.kv import KV
        from leopardai.queue import Queue
        ...
        if not self._loop_started:
            ...
            if self.queue_name is None:
                if os.environ.get("leopard_DEPLOYMENT_NAME") is None:
                    raise RuntimeError(
                        "queue_name is not set and leopard_DEPLOYMENT_NAME is not set"
                    )
                else:
                    self.queue_name = os.environ["leopard_DEPLOYMENT_NAME"]
            self._queue = Queue(
            ...
            if self.kv_name is None:
                if os.environ.get("leopard_DEPLOYMENT_NAME") is None:
                    raise RuntimeError(
                        "kv_name is not set and leopard_DEPLOYMENT_NAME is not set"
                    )
                else:
                    self.kv_name = os.environ["leopard_DEPLOYMENT_NAME"]
            self._kv = KV(
    ...
    def _worker_loop(self):
        ...
        while True:
            if self._leopard_worker_thread_should_exit:
            ...
            try:
                ...
                task_id = payload.pop(self.leopard_TASK_ID_TAG)
    ...
    def task_post(self, payload: Dict[str, Any]):
        ...
        payload[self.leopard_TASK_ID_TAG] = task_id
