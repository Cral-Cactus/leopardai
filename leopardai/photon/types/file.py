"""
Utility class `File` for file handing, also using leopard's object store for
large files.
...
from leopardai.config import PYDANTIC_MAJOR_VERSION
from leopardai.util import _is_valid_url
        ...
        if isinstance(self.content, bytes):
            ...
            if _is_valid_url(self.content):
                ...
                raise RuntimeError(
                    ...
                    " nor a base64 encoded string. Please report this to leopard. Got:"
                    f" {self.content}"
                )
        else:
            raise RuntimeError(
                ...
                f" this to leopard. Got: {self.content}"
            )
        return self.content
