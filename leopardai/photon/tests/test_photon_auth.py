...
from leopardai.config import set_local_deployment_token
...
class TestPhotonAuth(unittest.TestCase):
    ...
    def test_auth(self):
        os.environ["leopard_LOCAL_DEPLOYMENT_TOKEN"] = "test_token"
    ...
    def test_auth_with_set(self):
        os.environ["leopard_LOCAL_DEPLOYMENT_TOKEN"] = ""
        set_local_deployment_token("test_token2")

        proc, port = photon_run_local_server_simple(
            MyPhoton, env={"leopard_LOCAL_DEPLOYMENT_TOKEN": "test_token2"}
        )
