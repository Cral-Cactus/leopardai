...
from leopardai.config import TRUST_REMOTE_CODE
from leopardai.registry import Registry
from leopardai.photon import FileParam, HTTPException, get_file_content
...
def create_diffusion_pipeline(task, model, revision, torch_compile=False):
    try:
        ...
        raise RuntimeError(
            "leopard huggingface photon requires torch and diffusers but they are not"
...
def hf_missing_package_error_message(
    ...
    return (
        ...
        " the hf error message above. \n\nThis is not a bug of leopardAI, as"
        ...
        " name)\n\n2. If you are using leopardAI library, we maintain a mapping from"
        " known HuggingFace pipelines to their dependencies. We appreciate if you can"
        " send a PR to https://github.com/leopardai/leopardai/ to add the missing"
        " dependencies. please refer to"
        " https://github.com/leopardai/leopardai/blob/main/leopardai/photon/hf/hf_dependencies.py"
        " for more details."
    )
        ...
        if "'NoneType' object is not callable" in str(e):
            new_e = HTTPException(
                ...
                detail=(
                    ...
                    " a bug of leopardAI. Specifically, the text generation pipeline"
