...
class TestS3Cache(unittest.TestCase):
    def tearDown(self):
        # Remove the folder "/tmp/leopardai-s3cache-test" and any contents
        shutil.rmtree("/tmp/leopardai-s3cache-test", ignore_errors=True)
    ...
    def test_s3cache(self):
        ...
        from leopardai.util.s3cache import S3Cache

        cache = S3Cache(
            bucket="leopardai-s3cache-test", local_folder="/tmp/leopardai-s3cache-test"
        )
        # Test get
        ...
        with open("/tmp/leopardai-s3cache-test/answer.txt", "w") as f:
            f.write("1337\n")
        ...
        cache.rsync("")
        self.assertTrue(os.path.exists("/tmp/leopardai-s3cache-test/folder/foo.txt"))
        with open("/tmp/leopardai-s3cache-test/folder/foo.txt", "r") as f:
            self.assertEqual(f.read(), "bar\n")

        # Similar to get, we check file modifications
        with open("/tmp/leopardai-s3cache-test/folder/foo.txt", "w") as f:
            f.write("baz\n")
