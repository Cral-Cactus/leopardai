"""
leopard's KV API is a simple key-value store. It is useful for storing small pieces of data
...
from leopardai.api.v1.kv import ListKeysResponse
...
_leopard_max_key_length_ = 256
_leopard_max_value_length_ = 256 * 1024


class KV(object):
    """
    The leopard Key-Value store. Every named KV can be considered the equivalent of a KV / table / collection,
        ...
        if any(kv.metadata.name == name for kv in kvs):
            ...
            if create_if_not_exists:
                ...
                if wait_for_creation:
                    ...
                    while (
                        not self.ready()
                        and time.time() - start < _leopard_readiness_timeout_
                    ):
                        time.sleep(_leopard_readiness_wait_time_)
                    if not self.ready():
                        raise RuntimeError(
    ...
    async def async_wait(self):
        ...
        start = time.time()
        while not self.ready() and time.time() - start < _leopard_readiness_timeout_:
...
class KV(object):
    ...
    def put(self, key: str, value: Union[str, bytes]) -> None:
        ...
        """
        if len(key) > _leopard_max_key_length_:
            raise ValueError(
                f"Key length {len(key)} exceeds the maximum allowed length"
                f" {_leopard_max_key_length_}."
            )
        if len(value) > _leopard_max_value_length_:
            raise ValueError(
